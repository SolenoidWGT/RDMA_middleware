...
#define RDMA_SEND_THREASHOLD 524288
#endif
long count =0 ;
...
retry:
	...
	if(i!=index)
	{
		...
		// 分裂成左内存块和右内存块，将其order-1，再次加入都链表中
										...
										struct dhmp_mc_response* response)
{
	...
	/*
		当客户端请求的内存大小小于Area中伙伴分配器的最大块大小时，
		DHMP系统会直接在cur_area指针指向的Area实例中分配一个块来存储客户端的对象。
		注：max_index用来降低查询带来的开销
	*/
	...
	// 双向链表，area等于area_list表示没有找到满足条件的area
										...
										size_t length )
{
	...
	/*
		如果请求的内存大小大于最大块大小且小于Area大小，
		DHMP系统会直接分配一个固定大小的Area实例来满足内存分配请求，并且不会对其初始化一个伙伴分配器
	*/
...
static void dhmp_free_one_area(struct ibv_mr* mr)
{
	...
	if((&area->area_entry) != (&server->area_list))
	{
		...
		else
		{
			...
			numa_free(addr,mr->length);
			numa_free(area,sizeof(struct dhmp_area));
		}
	}
}
...
static struct dhmp_dram_entry *dhmp_create_new_dram_entry(void *nvm_addr, size_t length)
{
	...
	dram_entry=numa_alloc_onnode(sizeof(struct dhmp_dram_entry),server->config.mem_infos->dram_node);
	if(!dram_entry)
	{
		ERROR_LOG("allocate memory error.");
		goto out;
	...
	/*alloc dram memory*/
	dram_addr=numa_alloc_onnode(length,server->config.mem_infos->dram_node);
	if(!dram_addr)
	{
...
out_dram_addr:
	numa_free(dram_addr,length);
	
out_dram_entry:
	numa_free(dram_entry,sizeof(struct dhmp_dram_entry));
...
bool dhmp_destroy_dram_entry(void *nvm_addr)
{
	...
	if(dram_entry)
	{
		...
		numa_free(dram_addr,dram_entry->length);
		numa_free(dram_entry,sizeof(struct dhmp_dram_entry));
		return true;
	}
	return false;
...
static void dhmp_apply_dram_request_handler(struct dhmp_transport* rdma_trans, struct dhmp_msg* msg)
{
	...
	for(i=0;i<length;i++)
	{
		...
		if (dram_entry)
		{
			...
			//printf("^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^\n%p %d %d %d\n", dram_info[i].dram_mr.addr, *(int *)(dram_info[i].dram_mr.addr), *(int *)(tmp_head->nvm_addr), tmp_head->length);
...
//extern unsigned long get_mr_time;
